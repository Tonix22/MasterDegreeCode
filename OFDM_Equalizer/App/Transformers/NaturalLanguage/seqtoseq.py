...
for epoch in range(num_epochs):
    ...
    for batch_idx, batch in enumerate(train_iterator):
        ...
        inp_data = batch.src.to(device) # (word_length, batch_size)
        target   = batch.trg.to(device) # (word_length, batch_size)

        # Forward prop
        # target[:-1, :] remove the last row of the target tensor 
        # and all columns of the tensor
        # Last row is a padding (1) or is a eos (3)
        output = model(inp_data, target[:-1, :]) # output [target_seq_lenght-eos, batch, target_vocab_size] 
        ...
        output = output.reshape(-1, output.shape[2]) # then we merge frist two dim
